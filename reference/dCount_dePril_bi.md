...
Compute count probabilities using dePril convolution (section 3.2) for
the built-in distributions

Compute count probabilities using dePril convolution (section 3.2) for
the user passed survival function.
...
- cdfout:
  ...
  logical if `TRUE`, the cdf will be returned instead of the count
  probability.
...
The routine does minimum number of convolution using dePril trick to
compute the count probability P(x) sing `nsteps` steps, and refines
result by Richardson extrapolation if `extrap` is `TRUE` using the
